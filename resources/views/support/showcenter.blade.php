<x-app-layout>
 ...
 --}}
                    <div class="mb-4">
                        ...
                        <p><strong>Creado por:</strong> {{ $ticket->user->first_name }} {{ $ticket->user->last_name }}
                        </p>
                        ...
                        <div class="border-t border-gray-300 dark:border-gray-600 my-4 pt-4">
                            ...
                            <ul class="list-disc pl-5 mb-4">
                                @foreach ($ticket->assignedUsers as $assignment)
                                    <li>
                                        {{ $assignment->first_name }} {{ $assignment->last_name }} asignado el
                                        {{ $assignment->pivot->created_at }}: {{ $assignment->pivot->details }}
                            ...
                            <p class="text-red-500">EL ticket ya se encuentra asignado a un agente de soporte
                                </p>
                                    @else
                                @if (!in_array($ticket->state_id, [4, 7, 8]) && $ticket->is_active == 1)
                                    ...
                                    <form action="{{ route('support.store', $ticket) }}" method="POST"
                                        onsubmit="return confirm('¿Estás seguro de que deseas asignar el ticket?');">
                                        @csrf
                                        <div class="mb-4">
                                            <label for="user_id"
                                                class="block text-sm font-medium text-gray-700 dark:text-gray-300">Asignar
                                                a:</label>
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <select name="priority" id="priority"
                                                ...
                                                <option value="critical"
                                                    {{ $ticket->priority == 'critical' ? 'selected' : '' }}>Critical
                                                </option>
                                            </select>
                                        ...
                                        <div class="mb-4">
                                            <label for="details"
                                                class="block text-sm font-medium text-gray-700 dark:text-gray-300">Detalles:</label>
                                ...
                                @else
                                    <p class="text-red-500">No se puede asignar o reasignar el ticket en el estado actual.
                                    </p>
                                @endif
