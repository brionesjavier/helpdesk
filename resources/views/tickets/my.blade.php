                                ...
                                @forelse ($tickets as $index => $ticket)
                                    <tr class="{{ $index % 2 == 0 ? 'bg-gray-50 dark:bg-gray-700' : 'bg-gray-100 dark:bg-gray-800' }} hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-300">
                                        ...
                                        <td class="px-6 py-3 text-sm border-b border-gray-300 dark:border-gray-700 flex space-x-2">
                                            @can('tickets.show')
                                                <a href="{{ route('tickets.show', $ticket) }}" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded-md shadow-sm transition duration-300 ease-in-out">Ver</a>
                                            @endcan
                                            @can('tickets.destroy')
                                                @if ($ticket->state_id == 8 || $ticket->state_id == 7)
                                                    <form action="{{ route('tickets.destroy', $ticket) }}" method="POST" class="inline">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-md shadow-sm transition duration-300 ease-in-out">Eliminar</button>
                                                    </form>
                                                @endif
                                            @endcan
                                        </td>
