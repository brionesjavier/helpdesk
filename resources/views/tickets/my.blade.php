            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    @can('tickets.my.search')
                        ...
                        <form method="GET" action="{{ route('tickets.my') }}"
                            ...
                            <div class="flex flex-col md:flex-row md:items-end md:space-x-4 space-y-4">
                                ...
                                <div class="flex-none relative">
                                    ...
                                    <select id="sort_direction" name="sort_direction"
                                        class="block mt-1 w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-indigo-300 dark:focus:border-indigo-500 focus:ring focus:ring-indigo-200 dark:focus:ring-indigo-700 focus:ring-opacity-50">
                                        <option value="desc" class="inline-block ml-2"
                                            {{ request('sort_direction') == 'desc' ? 'selected' : '' }}>
                                            Descendente &#9660;
                                            <!-- Flecha hacia abajo -->
                                        </option>
                                        <option value="asc" class="inline-block ml-2"
                                            ...
                                            @can('tickets.destroy')
                                                @if ($ticket->state_id == 8 || $ticket->state_id == 7)
                                                    <form action="{{ route('tickets.destroy', $ticket) }}" method="POST"
                                                        class="inline"
                                                        onsubmit="return confirm('¿Estás seguro de que deseas quitar el ticket? Dejará de estar visible en esta lista.')">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                                                            class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-md shadow-sm transition duration-300 ease-in-out">Quitar</button>
                                                    </form>
                                                @endif
                                            @endcan
                                        </td>
